...
const saveData = (key, data) => { try { localStorage.setItem('bbt_' + key, JSON.stringify(data)); } catch {} };
const loadData = (key, defaultValue) => { try { const saved = localStorage.getItem('bbt_' + key); return saved ? JSON.parse(saved) : defaultValue; } catch { return defaultValue; } };
...
const styles = {
  ...
  statCard: (color, clickable = false) => ({
          ...
          {suggestions.map((loc, index) => (
            <div
              ...
              style={{
                padding: '16px 20px',
                cursor: 'pointer',
                background: selectedIndex === index ? colors.gray700 : 'transparent',
                borderBottom: index < suggestions.length - 1 ? `1px solid ${colors.gray700}` : 'none',
                display: 'flex',
                alignItems: 'center',
                gap: 12,
              }}
            ...
            >
              ...
              <div>
                <div style={{ color: colors.white, fontWeight: 600 }}>{loc.city}, {loc.state}</div>
                <div style={{ color: colors.gray400, fontSize: 13 }}>{STATE_NAMES[loc.state]}</div>
              </div>
            </div>
          ))}
        </div>
...
export default function App() {
  ...
  useEffect(() => { saveData('fuel', fuelEntries); }, [fuelEntries]);
  useEffect(() => { saveData('loads', loads); }, [loads]);
  useEffect(() => { saveData('ifta', iftaData); }, [iftaData]);
  useEffect(() => { saveData('expenses', expenses); }, [expenses]);
  useEffect(() => { saveData('perdiem', perDiemDays); }, [perDiemDays]);
  ...
  const LoadModal = () => {
    ...
    const handleStopChange = (index, field, value, locationData = null) => {
      ...
      // Auto-calculate loaded miles when stops have coordinates
      if (newStops.every(s => s.lat && s.lng)) {
      ...
      <div style={styles.modal} onClick={() => { setShowLoadModal(false); setEditingLoad(null); }}>
        <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ ...styles.card, background: colors.navyDark, marginBottom: 28, padding: 28 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <h3 style={{ color: colors.white, margin: 0, fontSize: 18 }}>📍 Route Stops</h3>
                <button type="button" onClick={addStop} style={{ ...styles.btn('secondary'), padding: '10px 16px', fontSize: 13 }}>
                  ➕ Add Stop
                </button>
              </div>
              ...
              {form.stops.map((stop, index) => (
                <div key={index} style={{ 
                  ...
                  padding: 20,
                  borderRadius: 14,
                  border: `1px solid ${index === 0 ? colors.green : index === form.stops.length - 1 ? colors.red : colors.orange}40`
                }}>
                  <div style={{ 
                    width: 40, 
                    height: 40, 
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, color: colors.gray400, marginBottom: 8, fontWeight: 600 }}>
                      {index === 0 ? '🚚 ORIGIN' : index === form.stops.length - 1 ? '🏁 DESTINATION' : `📦 STOP ${index}`}
                    </div>
                    <LocationAutocomplete
                      value={stop.location}
                      onChange={(val) => handleStopChange(index, 'location', val)}
                      onSelect={(loc) => handleStopChange(index, 'location', `${loc.city}, ${loc.state}`, loc)}
                      placeholder={`Enter ${index === 0 ? 'pickup' : index === form.stops.length - 1 ? 'delivery' : 'stop'} location...`}
                    />
                  </div>
                  {index > 0 && index < form.stops.length - 1 && (
                    <button type="button" onClick={() => removeStop(index)} style={{ 
                      background: colors.red, 
                      border: 'none', 
                      width: 36, 
                      height: 36, 
              ...
              {form.stops.length >= 2 && form.stops.every(s => s.lat && s.lng) && (
                <div style={{ 
                  background: `linear-gradient(135deg, ${colors.green}20 0%, ${colors.teal}20 100%)`,
                  padding: 16,
                  borderRadius: 12,
                  marginTop: 16,
                  display: 'flex',
                  ...
                  <div>
                    <div style={{ color: colors.white, fontWeight: 700 }}>
                      Estimated Route: {formatNumber(calculateRouteDistance(form.stops))} miles
                    </div>
                    <div style={{ color: colors.gray400, fontSize: 13 }}>
                      {form.stops.length} stops • Distance calculated automatically
                    </div>
                  </div>
                </div>
              )}
            ...
            {/* Deadhead Section */}
            <div style={{ ...styles.card, background: colors.navyDark, marginBottom: 28, padding: 28 }}>
              <h3 style={{ color: colors.white, margin: 0, marginBottom: 20, fontSize: 18 }}>🔄 Deadhead Miles</h3>
              <p style={{ color: colors.gray400, fontSize: 14, marginBottom: 20 }}>
                Empty miles driven to reach the pickup location (non-revenue miles).
              </p>
              
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20 }}>
                <div style={styles.formGroup}>
                  <label style={styles.label}>Coming From (Last Drop / Home Base)</label>
                  <LocationAutocomplete
                    value={form.deadheadOrigin}
                    onChange={(val) => setForm({...form, deadheadOrigin: val})}
                    onSelect={handleDeadheadOriginSelect}
                    placeholder="Where were you before this load?"
                  />
                </div>
                <div style={styles.formGroup}>
              ...
              {parseInt(form.deadheadMiles) > 0 && parseFloat(form.rate) > 0 && (
                <div style={{ 
                  background: colors.gray800,
                  padding: 16,
                  borderRadius: 12,
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 24, marginBottom: 28 }}>
              <div style={styles.formGroup}>
                ...
                <input type="number" value={form.loadedMiles} onChange={e => setForm({...form, loadedMiles: e.target.value})} placeholder="0" style={styles.input} />
              </div>
              <div style={styles.formGroup}>
                <label style={styles.label}>Fuel Cost ($)</label>
                <input type="number" step="0.01" value={form.fuelCost} onChange={e => setForm({...form, fuelCost: e.target.value})} placeholder="0.00" style={styles.input} />
              </div>
              <div style={styles.formGroup}>
                <label style={styles.label}>Other Expenses ($)</label>
                <input type="number" step="0.01" value={form.otherExpenses} onChange={e => setForm({...form, otherExpenses: e.target.value})} placeholder="0.00" style={styles.input} />
              </div>
            </div>
  ...
  const FuelModal = () => {
    ...
    const handleLocationSelect = (loc) => {
      setForm(prev => ({ ...prev, state: loc.state }));
    };
    ...
    return (
      <div style={styles.modal} onClick={() => { setShowFuelModal(false); setEditingFuel(null); }}>
        <div style={{ ...styles.modalContent, maxWidth: 550 }} onClick={e => e.stopPropagation()}>
          <h2 style={{ color: colors.white, marginBottom: 40, fontSize: 32 }}>
            {editingFuel ? '✏️ Edit Fuel Entry' : '⛽ Add Fuel Entry'}
          </h2>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
              <div style={styles.formGroup}>
                ...
                <input type="number" step="0.001" value={form.gallons} onChange={e => setForm({...form, gallons: e.target.value})} placeholder="0.000" style={styles.input} required />
              </div>
              <div style={styles.formGroup}>
            ...
            <div style={styles.formGroup}>
              <label style={styles.label}>Odometer (optional)</label>
              <input type="number" value={form.odometer} onChange={e => setForm({...form, odometer: e.target.value})} placeholder="Current mileage" style={styles.input} />
            </div>
            ...
            {form.gallons && form.pricePerGallon && (
              <div style={{ 
                ...
                <div style={{ color: colors.gray400, fontSize: 14 }}>Total Cost</div>
                <div style={{ color: colors.orange, fontSize: 32, fontWeight: 800 }}>
                  {formatCurrency((parseFloat(form.gallons) || 0) * (parseFloat(form.pricePerGallon) || 0))}
                </div>
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 16, marginTop: 36 }}>
              ...
              <button type="submit" style={styles.btn('primary')}>
                {editingFuel ? '💾 Save Changes' : '➕ Add Entry'}
              </button>
            </div>
          </form>
        </div>
  ...
  const IftaStateModal = () => {
    const [form, setForm] = useState({ state: '', miles: '', gallons: '' });
    ...
    return (
      <div style={styles.modal} onClick={() => setShowIftaModal(false)}>
        <div style={{ ...styles.modalContent, maxWidth: 500 }} onClick={e => e.stopPropagation()}>
          <h2 style={{ color: colors.white, marginBottom: 40, fontSize: 28 }}>📋 Add IFTA State Data</h2>
          ...
          <form onSubmit={handleSubmit}>
            <div style={styles.formGroup}>
              ...
              <select value={form.state} onChange={e => setForm({...form, state: e.target.value})} style={styles.select} required>
                <option value="">Select State</option>
                {Object.entries(STATE_NAMES).map(([code, name]) => (
                  <option key={code} value={code}>{name} ({code}) - ${IFTA_RATES[code]?.toFixed(3)}/gal</option>
                ))}
              </select>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
              <div style={styles.formGroup}>
                <label style={styles.label}>Miles Driven</label>
              ...
              <div style={styles.formGroup}>
                <label style={styles.label}>Gallons Purchased</label>
                <input type="number" step="0.001" value={form.gallons} onChange={e => setForm({...form, gallons: e.target.value})} placeholder="0.000" style={styles.input} required />
              </div>
            </div>

            <div style={{ display: 'flex', gap: 16, marginTop: 36 }}>
              <button type="button" onClick={() => setShowIftaModal(false)} style={styles.btn('secondary')}>Cancel</button>
              <button type="submit" style={styles.btn('primary')}>➕ Add State</button>
            </div>
          </form>
        </div>
      </div>
    ...
    <>
      ...
      <div style={styles.statsGrid}>
        ...
        <div style={styles.statCard(colors.yellow, true)} onClick={() => setActiveTab('loads')}>
          <div style={styles.statIcon}>📊</div>
          <div style={styles.statValue(colors.yellow)}>{formatCurrency(stats.profitPerMile)}</div>
          ...
          <table style={styles.table}>
            ...
            <tbody>
              {loads.slice(-5).reverse().map(load => {
                ...
                return (
                  <tr key={load.id}>
                    <td style={{ ...styles.td, borderRadius: '14px 0 0 14px' }}>{load.date}</td>
                    <td style={styles.td}>
                  ...
                  <tr key={load.id}>
                    ...
                    <td style={{ ...styles.td, borderRadius: '0 14px 14px 0' }}>
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button 
                          onClick={() => { setEditingLoad(load); setShowLoadModal(true); }}
  ...
  const renderFuel = () => (
    <>
      <div style={styles.header}>
        ...
        <p style={styles.pageSubtitle}>Track fuel purchases for IFTA reporting</p>
      </div>
      ...
      <div style={styles.card}>
        ...
        {fuelEntries.length > 0 ? (
          <table style={styles.table}>
            ...
            <tbody>
              {[...fuelEntries].reverse().map(entry => (
                <tr key={entry.id}>
                  <td style={{ ...styles.td, borderRadius: '14px 0 0 14px' }}>{entry.date}</td>
                  <td style={styles.td}>{entry.location || '-'}</td>
                  <td style={styles.td}>{entry.state}</td>
                  <td style={styles.td}>{formatNumber(entry.gallons, 3)}</td>
                  <td style={styles.td}>{formatCurrency(entry.pricePerGallon)}</td>
                  <td style={{ ...styles.td, color: colors.orange, fontWeight: 700 }}>
                    {formatCurrency(entry.gallons * entry.pricePerGallon)}
                  </td>
                  <td style={{ ...styles.td, borderRadius: '0 14px 14px 0' }}>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button 
                        onClick={() => { setEditingFuel(entry); setShowFuelModal(true); }}
                        style={{ ...styles.btn('secondary'), padding: '8px 12px', fontSize: 13 }}
                      >
                        ✏️
                      </button>
          ...
          <div style={{ textAlign: 'center', padding: '80px 20px', color: colors.gray400 }}>
            <div style={{ fontSize: 64, marginBottom: 24 }}>⛽</div>
            <p style={{ fontSize: 20, marginBottom: 8 }}>No fuel entries yet</p>
            <p style={{ fontSize: 14, marginBottom: 24 }}>Track your fuel purchases for IFTA</p>
            <button style={styles.btn('primary')} onClick={() => setShowFuelModal(true)}>
              ➕ Add First Entry
            </button>
          </div>
  ...
  // ============ IFTA VIEW ============
  const renderIFTA = () => {
    const quarterData = iftaData.filter(d => d.quarter === iftaQuarter);
    const totalMiles = quarterData.reduce((s, d) => s + d.miles, 0);
    const totalGallons = quarterData.reduce((s, d) => s + d.gallons, 0);
    const avgMPG = stats.fleetMPG || 6.5;
    
    const stateCalcs = quarterData.map(d => {
      ...
      return { ...d, gallonsUsed, taxOwed, taxPaid, netTax };
    });
    
    const totalTaxDue = stateCalcs.reduce((s, d) => s + d.netTax, 0);

    const exportCSV = () => {
      const rows = [
        ['IFTA Report', iftaQuarter],
        ['State', 'Miles', 'Gallons Purchased', 'Gallons Used', 'Tax Rate', 'Tax Owed', 'Tax Paid', 'Net Tax'],
        ...stateCalcs.map(d => [
          d.state, d.miles, d.gallons.toFixed(3), d.gallonsUsed.toFixed(3),
          d.taxRate.toFixed(3), d.taxOwed.toFixed(2), d.taxPaid.toFixed(2), d.netTax.toFixed(2)
        ]),
        [],
        ['Total', totalMiles, totalGallons.toFixed(3), '', '', '', '', totalTaxDue.toFixed(2)]
      ];
      const csv = rows.map(r => r.join(',')).join('\n');
      const blob = new Blob([csv], { type: 'text/csv' });
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = `IFTA-${iftaQuarter.replace(' ', '-')}.csv`;
      a.click();
    };
    ...
    return (
      <>
        <div style={styles.header}>
          <h1 style={styles.pageTitle}>IFTA Reporting</h1>
          <p style={styles.pageSubtitle}>International Fuel Tax Agreement calculations</p>
        </div>

        <div style={styles.statsGrid}>
          <div style={styles.statCard(colors.teal)}>
            <div style={styles.statIcon}>🛣️</div>
            <div style={styles.statValue(colors.teal)}>{formatNumber(totalMiles)}</div>
            <div style={styles.statLabel}>Total Miles</div>
          </div>
          <div style={styles.statCard(colors.orange)}>
            <div style={styles.statIcon}>⛽</div>
            <div style={styles.statValue(colors.orange)}>{formatNumber(totalGallons, 1)}</div>
            <div style={styles.statLabel}>Gallons Purchased</div>
          </div>
        ...
        <div style={styles.card}>
          <div style={styles.cardTitle}>
            <span>Fleet MPG: {avgMPG.toFixed(2)}</span>
            <div style={{ display: 'flex', gap: 16 }}>
              ...
              <button style={styles.btn('secondary')} onClick={exportCSV}>
                📄 Export CSV
              </button>
          ...
          {stateCalcs.length > 0 ? (
              ...
              <tbody>
                {stateCalcs.map(d => (
                  <tr key={d.id}>
                    ...
                    <td style={styles.td}>{formatNumber(d.gallons, 3)}</td>
                    <td style={styles.td}>{formatNumber(d.gallonsUsed, 3)}</td>
                    ...
                    <td style={{ ...styles.td, color: d.netTax >= 0 ? colors.red : colors.green, fontWeight: 700 }}>
                      {d.netTax >= 0 ? formatCurrency(d.netTax) : `(${formatCurrency(Math.abs(d.netTax))})`}
                    </td>
                    <td style={{ ...styles.td, borderRadius: '0 14px 14px 0' }}>
                      <button 
            ...
            <div style={{ textAlign: 'center', padding: '80px 20px', color: colors.gray400 }}>
              <div style={{ fontSize: 64, marginBottom: 24 }}>📋</div>
              <p style={{ fontSize: 20, marginBottom: 8 }}>No IFTA data for {iftaQuarter}</p>
              <p style={{ fontSize: 14, marginBottom: 24 }}>Add state-by-state miles and fuel purchases</p>
              <button style={styles.btn('primary')} onClick={() => setShowIftaModal(true)}>
                ➕ Add State Data
              </button>
            </div>
  ...
  const renderSettings = () => (
    <>
      <div style={styles.header}>
        ...
        <p style={styles.pageSubtitle}>Manage your app preferences and data</p>
      </div>
      ...
      <div style={styles.card}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 28, marginBottom: 36 }}>
          ...
          <div>
            <h3 style={{ color: colors.white, margin: 0, fontSize: 28 }}>
              BalanceBooks <span style={{ color: colors.orange }}>Trucking</span>
            </h3>
            <p style={{ color: colors.gray400, margin: '12px 0 0', fontSize: 16 }}>
              Version 1.2.0 • Owner-Operator Edition
            </p>
          </div>
        </div>
        <div style={{ background: colors.navyDark, borderRadius: 16, padding: 28, marginBottom: 28 }}>
          <p style={{ color: colors.gray300, lineHeight: 2, margin: 0, fontSize: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
          <span style={{ background: colors.green, color: colors.white, padding: '14px 28px', borderRadius: 14, fontSize: 16, fontWeight: 600 }}>🔒 100% Offline</span>
          <span style={{ background: colors.teal, color: colors.white, padding: '14px 28px', borderRadius: 14, fontSize: 16, fontWeight: 600 }}>🛡️ Privacy-First</span>
          <span style={{ background: colors.orange, color: colors.white, padding: '14px 28px', borderRadius: 14, fontSize: 16, fontWeight: 600 }}>🚛 Built for Truckers</span>
        </div>
      </div>

      <div style={styles.card}>
        <div style={styles.cardTitle}>Data Management</div>
        <p style={{ color: colors.gray400, marginBottom: 36, fontSize: 16 }}>
          Export or clear your data. Exports can be used to backup your information.
        </p>
        <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
          <button 
            style={styles.btn('primary')}
            onClick={() => {
              const data = { fuelEntries, loads, iftaData, expenses, perDiemDays, exportDate: new Date().toISOString() };
      ...
      <div style={styles.sidebar(sidebarCollapsed)}>
        ...
        {!sidebarCollapsed && (
          <div style={{ padding: '28px', borderTop: '1px solid rgba(255,255,255,0.1)', marginTop: 'auto' }}>
            <div style={{ fontSize: 13, color: colors.gray500, textAlign: 'center', lineHeight: 1.8 }}>
