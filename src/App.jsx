...
import { 
  ...
  iftaDB, 
  expensesDB, 
  perdiemDB, import React, { useState, useMemo, useEffect, useCallback, useRef } from 'react';
import { 
  db, 
  loadsDB, 
  fuelDB, 
  ...
  const renderReports = () => (
    <>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
        ...
        <button style={styles.btn(reportType === "trends" ? "primary" : "secondary")} onClick={() => setReportType("trends")}>Trends</button>
      </div>
      <div style={styles.card}>
        <p style={{ color: colors.white, padding: 20 }}>Report Type: {reportType}</p>
        <p style={{ color: colors.gray400, padding: 20 }}>Total Loads: {loads.length}</p>
      </div>
    </>
  );
