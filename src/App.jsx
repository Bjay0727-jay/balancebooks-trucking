...
import { 
  ...
  fuelDB, 
  iftaDB, 
  expensesDB, import React, { useState, useMemo, useEffect, useCallback, useRef } from 'react';
import { 
  db, 
  loadsDB, 
  fuelDB, 
...
export default function App() {
  ...
  const [reportType, setReportType] = useState("lanes");
  ...
  const renderDriverPay = () => {
    ...
    const filteredLoads = loads.filter(load => load.date >= startDate && load.date <= endDate);
    ...
    filteredLoads.forEach(load => {
      ...
      if (!driverPayMap[driverId]) {
        ...
        driverPayMap[driverId] = {
          ...
          loads: [], totalMiles: 0, totalRevenue: 0, totalPay: 0
        };
      }
      ...
      let pay = 0;
      if (driverData.paymentType === 'perMile') pay = miles * (parseFloat(driverData.payRate) || 0);
      else if (driverData.paymentType === 'percentage') pay = revenue * (parseFloat(driverData.payRate) || 0) / 100;
      else if (driverData.paymentType === 'flatRate') pay = parseFloat(driverData.payRate) || 0;
    ...
    const grandTotals = payStatements.reduce((acc, s) => ({
      loads: acc.loads + s.loads.length, miles: acc.miles + s.totalMiles,
      revenue: acc.revenue + s.totalRevenue, pay: acc.pay + s.totalPay
    }), { loads: 0, miles: 0, revenue: 0, pay: 0 });

    const setThisWeek = () => { const d = getWeekDates(); setPayPeriodStart(d.start); };
    const setLastWeek = () => {
      const now = new Date();
      const day = now.getDay();
      const monday = new Date(now);
    ...
    return (
      <>
        <div style={styles.header}>
          ...
          <p style={styles.pageSubtitle}>Period: {startDate} to {endDate}</p>
        </div>

        <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
          <button style={styles.btn('secondary')} onClick={setThisWeek}>This Week</button>
          <button style={styles.btn('secondary')} onClick={setLastWeek}>Last Week</button>
        </div>
        ...
        {payStatements.length === 0 ? (
          <div style={styles.card}>
            <div style={{ textAlign: 'center', padding: 60, color: colors.gray400 }}>
              ...
              <p style={{ fontSize: 14 }}>Add loads with assigned drivers to see pay statements</p>
            </div>
          </div>
        ) : (
          payStatements.map(stmt => (
            <div key={stmt.driver.id} style={{ ...styles.card, marginBottom: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, paddingBottom: 16, borderBottom: '1px solid ' + colors.gray700 }}>
                <div>
                  <h3 style={{ color: colors.white, fontSize: 18, marginBottom: 4 }}>{stmt.driver.firstName} {stmt.driver.lastName}</h3>
                  <p style={{ color: colors.gray400, fontSize: 13 }}>
                    Pay: <span style={{ color: colors.teal }}>{getPayLabel(stmt.driver.paymentType, stmt.driver.payRate)}</span>
                    {' • '}{stmt.loads.length} loads • {formatNumber(stmt.totalMiles)} mi
                  </p>
                </div>
                ...
                <tbody>
                  {stmt.loads.map(load => (
                    <tr key={load.id}>
                      ...
                      <td style={styles.td}>{formatNumber((parseFloat(load.loadedMiles) || 0) + (parseFloat(load.deadheadMiles) || 0))}</td>
  ...
  const renderReports = () => {
    // Calculate lane profitability
    const laneData = {};
    loads.forEach(load => {
      const origin = load.stops && load.stops[0] ? load.stops[0].location : 'Unknown';
      const dest = load.stops && load.stops.length > 0 ? load.stops[load.stops.length - 1].location : 'Unknown';
      ...
      if (!laneData[lane]) {
        laneData[lane] = { lane: lane, loads: 0, miles: 0, revenue: 0 };
      }
      laneData[lane].loads += 1;
      laneData[lane].miles += miles;
      laneData[lane].revenue += revenue;
    });
    const laneStats = Object.values(laneData)
      .map(l => ({ ...l, rpm: l.miles > 0 ? l.revenue / l.miles : 0 }))
      .sort((a, b) => b.revenue - a.revenue);

    // Calculate broker profitability
    const brokerData = {};
    loads.forEach(load => {
      ...
      if (!brokerData[broker]) {
        brokerData[broker] = { broker: broker, loads: 0, miles: 0, revenue: 0 };
      }
      brokerData[broker].loads += 1;
      brokerData[broker].miles += miles;
      brokerData[broker].revenue += revenue;
    });
    const brokerStats = Object.values(brokerData)
      .map(b => ({ ...b, rpm: b.miles > 0 ? b.revenue / b.miles : 0 }))
      .sort((a, b) => b.revenue - a.revenue);

    // Calculate monthly trends
    const monthlyData = {};
    loads.forEach(load => {
      const month = load.date ? load.date.substring(0, 7) : 'Unknown';
      const miles = (parseFloat(load.loadedMiles) || 0) + (parseFloat(load.deadheadMiles) || 0);
      const revenue = parseFloat(load.rate) || 0;
      
      if (!monthlyData[month]) {
        monthlyData[month] = { month: month, loads: 0, miles: 0, revenue: 0 };
      }
      monthlyData[month].loads += 1;
      monthlyData[month].miles += miles;
      monthlyData[month].revenue += revenue;
    });
    const monthlyStats = Object.values(monthlyData)
      .map(m => ({ ...m, rpm: m.miles > 0 ? m.revenue / m.miles : 0 }))
      .sort((a, b) => a.month.localeCompare(b.month));

    const totals = loads.reduce((acc, load) => {
      const miles = (parseFloat(load.loadedMiles) || 0) + (parseFloat(load.deadheadMiles) || 0);
      const revenue = parseFloat(load.rate) || 0;
      return { loads: acc.loads + 1, miles: acc.miles + miles, revenue: acc.revenue + revenue };
    ...
    return (
      <>
        <div style={styles.header}>
          <h1 style={styles.pageTitle}>Profitability Reports</h1>
          <p style={styles.pageSubtitle}>Analyze your business performance</p>
        </div>

        <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
          <button style={styles.btn(reportType === 'lanes' ? 'primary' : 'secondary')} onClick={() => setReportType('lanes')}>By Lane</button>
          <button style={styles.btn(reportType === 'brokers' ? 'primary' : 'secondary')} onClick={() => setReportType('brokers')}>By Broker</button>
          <button style={styles.btn(reportType === 'trends' ? 'primary' : 'secondary')} onClick={() => setReportType('trends')}>Monthly Trends</button>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
          <div style={styles.statCard()}>
            <div style={styles.statLabel}>Total Loads</div>
            <div style={styles.statValue(colors.teal)}>{totals.loads}</div>
          </div>
          <div style={styles.statCard()}>
            <div style={styles.statLabel}>Total Miles</div>
            <div style={styles.statValue(colors.orange)}>{formatNumber(totals.miles)}</div>
          </div>
          <div style={styles.statCard()}>
            <div style={styles.statLabel}>Total Revenue</div>
            <div style={styles.statValue(colors.green)}>{formatCurrency(totals.revenue)}</div>
          </div>
          <div style={styles.statCard()}>
            <div style={styles.statLabel}>Avg $/Mile</div>
            <div style={styles.statValue(colors.blue)}>{formatCurrency(totals.rpm)}</div>
          </div>
        </div>

        {reportType === 'lanes' && (
          <div style={styles.card}>
            <h3 style={{ color: colors.white, marginBottom: 16 }}>Profitability by Lane</h3>
            {laneStats.length === 0 ? (
              <p style={{ color: colors.gray400, textAlign: 'center', padding: 40 }}>No lane data available</p>
            ) : (
              <table style={styles.table}>
                <thead>
                  <tr>
                ...
                <tbody>
                  {laneStats.slice(0, 20).map((lane, idx) => (
                    <tr key={idx}>
                      <td style={styles.td}>{lane.lane}</td>
                      <td style={styles.td}>{lane.loads}</td>
                      <td style={styles.td}>{formatNumber(lane.miles)}</td>
                      <td style={{ ...styles.td, color: colors.green }}>{formatCurrency(lane.revenue)}</td>
                      <td style={{ ...styles.td, color: lane.rpm >= totals.rpm ? colors.green : colors.red }}>{formatCurrency(lane.rpm)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {reportType === 'brokers' && (
          <div style={styles.card}>
            <h3 style={{ color: colors.white, marginBottom: 16 }}>Profitability by Broker</h3>
            {brokerStats.length === 0 ? (
              <p style={{ color: colors.gray400, textAlign: 'center', padding: 40 }}>No broker data available</p>
            ) : (
              <table style={styles.table}>
                <thead>
                  <tr>
                ...
                <tbody>
                  {brokerStats.map((broker, idx) => (
                    <tr key={idx}>
                      <td style={styles.td}>{broker.broker}</td>
                      <td style={styles.td}>{broker.loads}</td>
                      <td style={styles.td}>{formatNumber(broker.miles)}</td>
                      <td style={{ ...styles.td, color: colors.green }}>{formatCurrency(broker.revenue)}</td>
                      <td style={{ ...styles.td, color: broker.rpm >= totals.rpm ? colors.green : colors.red }}>{formatCurrency(broker.rpm)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {reportType === 'trends' && (
          <div style={styles.card}>
            <h3 style={{ color: colors.white, marginBottom: 16 }}>Monthly Revenue Trends</h3>
            {monthlyStats.length === 0 ? (
              <p style={{ color: colors.gray400, textAlign: 'center', padding: 40 }}>No trend data available</p>
            ) : (
              <table style={styles.table}>
                <thead>
                  <tr>
                ...
                <tbody>
                  {monthlyStats.map((month, idx) => (
                    <tr key={idx}>
                      ...
                      <td style={{ ...styles.td, color: month.rpm >= totals.rpm ? colors.green : colors.red }}>{formatCurrency(month.rpm)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
